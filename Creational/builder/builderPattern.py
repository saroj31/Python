"""
This pattern is to create complex Objects in a step by step manner. When we have a very complex object and we need user inputs to build those objects then
we can make a minimal object and then build each step slowly by user requirements.

It is similar to factory pattern but factory gives us the object in one shot but here it happens in steps as the object is very complicated.
"""
...
# at the initial point a builder pattern makes a minimal pizza and then later its builds its parts step by step
class Pizza:
...
# this is the main client of builder code
class Waiter:
    def __init__(self):
        self.builder = None
